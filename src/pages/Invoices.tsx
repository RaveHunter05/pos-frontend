...
export default function Invoices() {
	...
	const invoicesQuery = useQuery({
		...
		queryFn: async () => {
			...
			console.log({ responseInvoices: response });
			return response;
		},
	});
	...
	useEffect(() => {
		...
		if (
			!selectedId ||
			!filteredInvoices.some((invoice) => invoice.id === selectedId)
		) {
	...
	const selectedInvoice = useMemo(() => {
		...
		return (
			filteredInvoices.find((invoice) => invoice.id === selectedId) ??
			filteredInvoices[0]
		);
	}, [filteredInvoices, selectedId]);
				...
				<aside className="bg-white rounded-xl shadow-sm p-6">
						...
						{filteredInvoices.map((invoice) => (
							<li key={invoice.id}>
								<button
									...
									className={`w-full px-3 py-2 rounded-lg text-left transition-colors ${selectedInvoice?.id === invoice.id
					...
					{selectedInvoice && (
						<div className="flex flex-col gap-2">
							...
							<p className="text-sm">
								<strong>Estado:</strong> {selectedInvoice.status}
							</p>
							<p className="text-sm">
								<strong>Método de pago:</strong> {selectedInvoice.paymentMethod}
							</p>
							<p className="text-sm">
								<strong>Total:</strong>{' '}
