...
export function TopProductsChart({
	...
	const topProductsQuery = useQuery({
		...
		queryFn: async (): Promise<TopProductPoint[]> => {
			...
			orders.forEach((order) => {
				...
				if (
					orderDate &&
					(orderDate.isBefore(rangeFrom, 'day') ||
						orderDate.isAfter(rangeTo, 'day'))
				) {
					return;
				}
	...
	const chartData = useMemo(() => {
		...
		const valueFor = (item: TopProductPoint) =>
			metric === 'quantity' ? item.quantity : item.total;
		return items
			.slice()
	...
	const totalMetric = useMemo(
		() =>
			chartData.reduce(
				(acc, item) =>
					acc + (metric === 'quantity' ? item.quantity : item.total),
				0,
			),
		[chartData, metric],
	);
	...
	const isEmpty =
		!topProductsQuery.isLoading && (!chartData.length || totalMetric <= 0);
